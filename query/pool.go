...
func (p *Pool[K, N]) Advance(ctx context.Context, ev PoolEvent) PoolState {
	...
	case *EventPoolMessageResponse[K, N]:
		if qry, ok := p.queryIndex[tev.QueryID]; ok {
			state, terminal := p.advanceQuery(ctx, qry, &EventQueryMessageResponse[K, N]{
				NodeID:   tev.NodeID,
	...
	case *EventPoolMessageFailure[K, N]:
		if qry, ok := p.queryIndex[tev.QueryID]; ok {
			state, terminal := p.advanceQuery(ctx, qry, &EventQueryMessageFailure[K, N]{
				NodeID: tev.NodeID,
				Error:  tev.Error,
			})
			if terminal {
...
func (p *Pool[K, N]) advanceQuery(ctx context.Context, qry *Query[K, N], qev QueryEvent) (PoolState, bool) {
	...
	case *StateQueryWaitingMessage[K, N]:
		p.queriesInFlight++
		return &StatePoolQueryMessage[K, N]{
			QueryID:    st.QueryID,
			Stats:      st.Stats,
			NodeID:     st.NodeID,
...
func (p *Pool[K, N]) addQuery(ctx context.Context, queryID QueryID, target K, protocolID address.ProtocolID, msg kad.Request[K, N], knownClosestNodes []N) error {
...
type StatePoolQueryMessage[K kad.Key[K], N kad.NodeID[K]] struct {
	QueryID    QueryID
	NodeID     N
...
func (*StatePoolQueryMessage[K, N]) poolState()  {}
...
type EventPoolAddQuery[K kad.Key[K], N kad.NodeID[K]] struct {
	...
	Message           kad.Request[K, N]  // the message the query should send to each node it traverses
...
type EventPoolMessageResponse[K kad.Key[K], N kad.NodeID[K]] struct {
...
func (*EventPoolMessageResponse[K, N]) poolEvent() {}
