...
type RequestHandlerFn[K kad.Key[K]] func(context.Context, address.NodeID[K],
...
// known remote peers.
type NetworkedEndpoint[K kad.Key[K]] interface {
	Endpoint[K]
	// Connectedness returns the connectedness of the given peer.
	Connectedness(address.NodeID[K]) (network.Connectedness, error)
}
...
type SimEndpoint[K kad.Key[K]] interface {
	ServerEndpoint[K]
	// HandleMessage handles a message from the given peer.
	HandleMessage(context.Context, address.NodeID[K], address.ProtocolID,
