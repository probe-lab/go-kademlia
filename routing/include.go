...
func (b *Include[K, N]) Advance(ctx context.Context, ev IncludeEvent) IncludeState {
	...
	case *EventIncludeMessageResponse[K, N]:
	...
	return &StateIncludeFindNodeMessage[K, N]{
		NodeID: candidate,
	}
}
...
type StateIncludeFindNodeMessage[K kad.Key[K], N kad.NodeID[K]] struct {
	NodeID N // the node to send the mssage to
}
...
func (*StateIncludeFindNodeMessage[K, N]) includeState() {}
...
type EventIncludeMessageResponse[K kad.Key[K], N kad.NodeID[K]] struct {
...
func (*EventIncludeMessageResponse[K, N]) includeEvent() {}
