...
var kadRemotePeers = []address.NodeAddr[key.Key8]{
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b10001000)), nil), // 1000 1000 (bucket 0)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b11010010)), nil), // 1101 0010 (bucket 0)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b01001011)), nil), // 0100 1011 (bucket 1)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b01010011)), nil), // 0101 0011 (bucket 1)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b00101110)), nil), // 0010 1110 (bucket 2)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b00110110)), nil), // 0011 0110 (bucket 2)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b00011111)), nil), // 0001 1111 (bucket 3)
	kadaddr.NewKadAddr(kadid.NewKadID(key.Key8(0b00010001)), nil), // 0001 0001 (bucket 3)
...
func TestMessageHandling(t *testing.T) {
	...
	order := []address.NodeAddr[key.Key8]{
	...
	order = []address.NodeAddr[key.Key8]{kadRemotePeers[2], kadRemotePeers[3], kadRemotePeers[4]}
	for i, p := range resp.CloserNodes() {
		require.Equal(t, order[i], p)
	}
}
...
func TestRequestNoNetworkAddress(t *testing.T) {
	...
	success, err := rt.AddPeer(ctx, node)
	require.NoError(t, err)
	require.True(t, success)
